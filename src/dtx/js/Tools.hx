...
class Tools {
	...
	**/
	public static inline function toCollection(n:DOMNode):DOMCollection {
		return dtx.std.Tools.toCollection(n);
	}
	...
	**/
	public static function parse(xml:String):DOMCollection {
		...
		if (xml != null && xml != "") {
			var parentTag = "div";
			if (dtx.std.Tools.firstTag.match(xml)) {
				// It begins with a
				var tagName = dtx.std.Tools.firstTag.matched(1);
	...
	**/
	public static inline function setDocument(newDocument:DOMNode):Void
	{
